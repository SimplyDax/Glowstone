...
class RCON{
	...
	public function __construct(Server $server, $password, $port = 19132, $interface = "0.0.0.0", $threads = 1, $clientsPerThread = 50){
		...
		$this->server->getLogger()->info("Запуск Rcon...");
		if($this->password === ""){
			$this->server->getLogger()->critical("Rcon не смог запуститься: нету пароля");
		...
		if($this->socket === false or !socket_bind($this->socket, $interface, (int) $port) or !socket_listen($this->socket)){
			$this->server->getLogger()->critical("Rcon не смог запуститься: " . socket_strerror(socket_last_error()));
			$this->threads = 0;
			return;
		}
		...
		$this->server->getLogger()->info("Rcon запущен на: $addr:$port");
	}
