...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
	...
	if(!isset($this->cps[time()])) return 0;
	...
	public function setBanned($value){
		if($value === true){
			...
			$this->kick("§fYour §enickname §fwas permanently §cbanned");
		}else{
	...
	public function handleDataPacket(DataPacket $packet){
		...
		switch($packet::NETWORK_ID){
			...
			case ProtocolInfo::LOGIN_PACKET:
				...
				if(count($this->server->getOnlinePlayers()) >= $this->server->getMaxPlayers() and $this->kick("§c» §fСервер заполнен!", false)){
					break;
				}
	...
	/**
	 * Bugged on 0.14.2+, on client side :(
	 * 
	 *
	 * @param $message
	 * @return bool
